...
// constructor for game engine class. Reads the maps and assigns them to
// variables.
...
// return map variable
int *GameEngine::getMap() { return *map; }
...
// for each tile, check if is forming a triad (vertical or horizontal). If yes,
// this should be resolved.
bool GameEngine::needsUpdate() {
  for (int x = 0; x < 10; x++) {
    for (int y = 0; y < 10; y++) {
      if ((x < 8 && map[x][y] == map[x + 1][y] && map[x][y] == map[x + 2][y] &&
           map[x][y] != 0) ||
          (y < 8 && map[x][y] == map[x][y + 1] && map[x][y] == map[x][y + 2] &&
           map[x][y] != 0)) {
        return true;
      }
    }
...
// check whether a move is legal (it is legal if it results in a triad, vertical
// or horizontal)
...
// check if two tiles are next to each other (engine doesn't allow swap between
// non-adjacent tiles)
...
bool GameEngine::isVerticalLegal(int x1, int y1, int x2, int y2) {
  if (((map[x2][y2] == map[x1][y1 - 2]) && (map[x2][y2] == map[x1][y1 - 1]) &&
      ...
      ((map[x1][y1] == map[x2][y2 - 2]) && (map[x1][y1] == map[x2][y2 - 1]) &&
       y2 > 1 && y1 >= y2)) {
    return true;
  } else if (((map[x2][y2] == map[x1][y1 + 2]) &&
              (map[x2][y2] == map[x1][y1 + 1]) && y1 < 8 && y1 >= y2) ||
             ((map[x1][y1] == map[x2][y2 + 2]) &&
              (map[x1][y1] == map[x2][y2 + 1]) && y2 < 8 && y2 >= y1)) {
    return true;
  } else if (x1 != x2 &&
             (((map[x2][y2] == map[x1][y1 - 1]) &&
               (map[x2][y2] == map[x1][y1 + 1]) && (y1 > 0 && y1 < 9)) ||
              ((map[x1][y1] == map[x2][y2 - 1]) &&
               (map[x1][y1] == map[x2][y2 + 1]) && (y2 > 0 && y2 < 9)))) {
    return true;
  }
  return false;
}
...
bool GameEngine::isHorizontalLegal(int x1, int y1, int x2,
                                   int y2) { // check before swap is made
  ...
  else if (((map[x2][y2] == map[x1 + 2][y1]) &&
            (map[x2][y2] == map[x1 + 1][y1]) && x1 < 8 && x1 >= x2) ||
           ((map[x1][y1] == map[x2 + 2][y2]) &&
            (map[x1][y1] == map[x2 + 1][y2]) && x2 < 8 && x2 >= x1)) {
    return true;
  }

  else if (y1 != y2 &&
           (((map[x2][y2] == map[x1 - 1][y1]) &&
             (map[x2][y2] == map[x1 + 1][y1]) && (x1 > 0 && x1 < 9)) ||
            ((map[x1][y1] == map[x2 - 1][y2]) &&
             (map[x1][y1] == map[x2 + 1][y2]) && (x2 > 0 && x2 < 9)))) {
    return true;
  }
  return false;
}

// check if the current map state has any legal moves (moves that would lead to
// triads, horizontal or vertical)
bool GameEngine::hasLegalMoves() {
  for (int x = 0; x < 10; x++) {
    for (int y = 0; y < 10; y++) {
      if ((x != 9 && isLegalMove(x, y, x + 1, y)) ||
          (y != 9 && isLegalMove(x, y, x, y + 1))) {
        return true;
      }
    }
  }
...
void GameEngine::breakVertical(int x1, int y1, int points) {
  int x2 = x1 + 1,
      count = 1; // y2 is always equal to y1 (because we're looking vertically).
                 // count=1 because we have at least 1 tile. x2 starts as x1+1
  while (map[x2][y1] != 0 &&
         map[x2][y1] ==
             map[x1][y1]) { // while tiles are equal to (x1 y1) starting from
                            // the one next to it (x2 y1).
...
void GameEngine::breakHorizontal(int x1, int y1, int points) {
  ...
  if (y2 < 8 && x1 <= 9) { // if there is still room, horizontally and
                           // vertically
...
void GameEngine::breakStuff(int x, int y, int points) {
  ...
  // nothing should happen in between here. breaks happen "simultaneously" (in
  // the same game loop iteration)
...
// return score
int GameEngine::getScore() { return totalScore; }

// add points to score
void GameEngine::addScore(int points) { totalScore += points; }
